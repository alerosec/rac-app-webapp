...
function updateAccountLabel() {
  if (currentUser?.email) {
    accountName.textContent = currentUser.email;
  } else {
...
const uploadEvidenceButton = document.querySelector("#uploadEvidence");
if (uploadEvidenceButton) {
  uploadEvidenceButton.addEventListener("click", () => {
    if (!ensureAuth()) return;
    state.evidence.unshift({
      id: crypto.randomUUID ? crypto.randomUUID() : `e-${Date.now()}`,
      type: "PDF",
      name: "Evidence upload placeholder",
      meta: `Captured by ${currentUser?.email || "authenticated user"} · pending review`,
      tag: "Pending",
    });
    refreshDashboard();
    showToast("Evidence upload will be implemented in the production workflow.");
  });
}
